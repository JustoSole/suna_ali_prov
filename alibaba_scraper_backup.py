...
def max_price_from_areacontent(area):
    ...
    Estrategia: extraer todos los números, normalizar y devolver el máximo.
    """
    nums = []
    for raw in re.findall(r'[-+]?\d[\d.,]*', area):
        # Normaliza: si hay coma y punto, asumimos coma=separador de miles y punto=decimal.
        # Si hay solo coma, la tratamos como miles (Alibaba global suele usar punto decimal).
        cleaned = raw.replace(',', '')
        try:
...
def extract_product_from_card(card):
    ...
    if price_elem:
        ...
        prices = extract_price_range_from_text(price_text)
        if prices:
            if len(prices) == 1:
    ...
    if review_elem:
        ...
        if content_match:
            ...
            if '@@' in review_data:
                ...
                try:
                    product['product_review_avg'] = float(parts[0])
                    product['product_review_count'] = int(parts[1])
                except:
                    pass
...
# MÉTODO ANTERIOR - MANTENIDO COMO FALLBACK
def extract_alibaba_reviews_prices(html):
    ...
    for rev in soup.select('[data-aplus-auto-card-mod^="area=review"]'):
        ...
        # Ubicar el anchor de detalle más cercano (la misma tarjeta)
        link_tag = rev.find_parent().find('a', class_='search-card-e-detail-wrapper')
        if not link_tag:
            link_tag = rev.find_parent('div').find('a', class_='search-card-e-detail-wrapper')
    ...
    for price in soup.select('[data-aplus-auto-card-mod^="area=price"]'):
        ...
        link_tag = price.find_parent().find('a', class_='search-card-e-detail-wrapper')
        if not link_tag:
            link_tag = price.find_parent('div').find('a', class_='search-card-e-detail-wrapper')
        if link_tag and link_tag.get('href'):
            href = link_tag['href']
            if not href.startswith('http'):
                href = 'https:' + href if href.startswith('//') else href
...
class AlibabaProductScraper:
    ...
    def parse_offer(self, offer):
        try:
            ...
            # precio → manejo de rangos y moneda (mejorado)
            price = offer.get('price') or offer.get('priceV2')
            if price:
                price_str = str(price)
                prices = self.extract_price_range(price_str)
                if prices:
                    if len(prices) == 1:
                        product['price'] = prices[0]
                        product['price_min'] = prices[0]
            ...
            # reviews - MEJORADO: Extraer desde la estructura real del JSON
            # Reviews están anidados en diferentes lugares
            review_count = 0
            review_score = 0.0
            
            # Método 1: reviewCount y reviewScore directos
            if offer.get('reviewCount') is not None:
                try:
                    review_count = int(offer.get('reviewCount', 0))
                except (ValueError, TypeError):
                    review_count = 0
                    
            if offer.get('reviewScore') is not None:
                try:
                    review_score = float(offer.get('reviewScore', 0))
                except (ValueError, TypeError):
                    review_score = 0.0
            
            # Método 2: Buscar en iuiInfo.dataSource.companyInfo (estructura anidada)
            if review_count == 0 or review_score == 0.0:
                iui_info = offer.get('iuiInfo', {})
                if isinstance(iui_info, dict):
                    data_source = iui_info.get('dataSource', {})
                    if isinstance(data_source, dict):
                        company_info = data_source.get('companyInfo', {})
                        if isinstance(company_info, dict):
                            if review_count == 0 and company_info.get('reviewCount'):
                                try:
                                    review_count = int(company_info.get('reviewCount', 0))
                                except (ValueError, TypeError):
                                    pass
                            if review_score == 0.0 and company_info.get('reviewScore'):
                                try:
                                    review_score = float(company_info.get('reviewScore', 0))
                                except (ValueError, TypeError):
                                    pass
            
            # Establecer campos de reviews (nuevos y compatibilidad)
            if review_count > 0:
                product['product_review_count'] = review_count
                product['amount_of_reviews'] = review_count  # Compatibilidad
            if review_score > 0:
                product['product_review_avg'] = review_score
                product['review_average'] = review_score  # Compatibilidad
